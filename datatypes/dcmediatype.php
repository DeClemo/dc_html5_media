<?php
/**
 * File containing the dcMediaType class.
 *
...
/*!
  \class dcMediaType dcmediatype.php
  \ingroup eZDatatype
  \brief The class dcMediaType handles storage and playback of media files.

*/

class dcMediaType extends eZDataType
{
    const DATA_TYPE_STRING = "dcmedia";
    const MAX_FILESIZE_FIELD = 'data_int1';
    const MAX_FILESIZE_VARIABLE = '_dcmedia_max_filesize_';
    const TYPE_FIELD = "data_text1";
    const TYPE_VARIABLE = "_dcmedia_type_";

    function dcMediaType()
    {
    ...
    */
    function postInitializeObjectAttribute( $contentObjectAttribute, $currentVersion, $originalContentObjectAttribute )
    {
        if ( $currentVersion != false )
        {
            ...
            $oldfile = dcMedia::fetch( $contentObjectAttributeID, $currentVersion );
            if( $oldfile != null )
            {
        ...
        else
        {
            ...
            $media = dcMedia::create( $contentObjectAttributeID, $version );

            $contentClassAttribute = $contentObjectAttribute->contentClassAttribute();
            $pluginPage = dcMediaType::pluginPage( $contentClassAttribute->attribute( 'data_text1' ) );
    ...
    */
    function trashStoredObjectAttribute( $contentObjectAttribute, $version = null )
    {
        ...
        if ( $version == null )
            $mediaFiles = dcMedia::fetch( $contentObjectAttributeID, null );
        else
            $mediaFiles = array( dcMedia::fetch( $contentObjectAttributeID, $version ) );
        ...
        foreach ( $mediaFiles as $mediaFile )
        {
            ...
            // Check if there are any other records in dcmedia that point to that fileName.
            $mediaObjectsWithSameFileName = dcMedia::fetchByFileName( $fileName );
...
//                $orig_dir = "var/storage/original/" . $prefix;
        ...
        {
            $mediaFiles = dcMedia::fetchByContentObjectID( $contentObjectAttribute->attribute( 'contentobject_id' ) );
            $count = 0;
            $currentBinaryFile = dcMedia::fetch( $contentObjectAttributeID, $version );
...
//              $orig_dir = "var/storage/original/" . $prefix;
        ...
        dcMedia::removeByID( $contentObjectAttributeID, $version );
    }
    ...
    */
    function validateObjectAttributeHTTPInput( $http, $base, $contentObjectAttribute )
    {
        ...
        if ( $contentObjectAttribute->validateIsRequired() )
        {
            $contentObjectAttributeID = $contentObjectAttribute->attribute( "id" );
            $version = $contentObjectAttribute->attribute( "version" );
            $media = dcMedia::fetch( $contentObjectAttributeID, $version );
    ...
    */
    function checkFileUploads()
    {
        ...
        {
            if ( empty( $GLOBALS['dcMediaTypeWarningAdded'] ) )
            {
                ...
                $GLOBALS['dcMediaTypeWarningAdded'] = true;
            }
        }
    }
    ...
    */
    function fetchObjectAttributeHTTPInput( $http, $base, $contentObjectAttribute )
    {
        ...
        dcMediaType::checkFileUploads();

        $classAttribute = $contentObjectAttribute->contentClassAttribute();
        $player = $classAttribute->attribute( "data_text1" );
        $pluginPage = dcMediaType::pluginPage( $player );
    ...
    */
    ...
    /*!
     Inserts the file using the dcMedia class.
    */
                             ...
                             &$result )
    {
        ...
        $media = dcMedia::fetch( $attributeID, $objectVersion );
        if ( $media === null )
            $media = dcMedia::create( $attributeID, $objectVersion );

        $httpFile->setMimeType( $mimeData['name'] );
        if ( !$httpFile->store( "original", false, false ) )
        {
            $result['errors'][] = array( 'description' => ezpI18n::tr( 'kernel/classes/datatypes/dcmedia',
    ...
    */
    function storedFileInformation( $object, $objectVersion, $objectLanguage,
                                    $objectAttribute )
    {
        $mediaFile = dcMedia::fetch( $objectAttribute->attribute( "id" ),
    ...
    */
    ...
    {
        $mediaFile = dcMedia::fetch( $contentObjectAttribute->attribute( "id" ),
    ...
    */
    ...
    function unserializeContentObjectAttribute( $package, $objectAttribute, $attributeNode )
    {
        ...
        $mediaFile = dcMedia::create( $objectAttribute->attribute( 'id' ), $objectAttribute->attribute( 'version' ) );
